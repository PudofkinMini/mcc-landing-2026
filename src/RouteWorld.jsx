...
const processCurve = new THREE.CatmullRomCurve3(
  [
    new THREE.Vector3(-19.3, 0.94, 0),
    new THREE.Vector3(-16.7, 0.94, 0),
    new THREE.Vector3(-14.7, 0.94, 0),
    new THREE.Vector3(-12.6, 0.91, 0),
    new THREE.Vector3(-11.35, 0.55, 0),
    new THREE.Vector3(-10.25, 0.42, 0),
  ],
  false,
  'centripetal',
)
...
const truckCurves = customerSites.map((site, index) => {
  ...
  return new THREE.CatmullRomCurve3(
    [
      new THREE.Vector3(-6.15, 0.48, startZ),
      new THREE.Vector3(-3.2, 0.48, startZ),
      new THREE.Vector3(-0.8, 0.48, startZ + (site.lane - startZ) * 0.24),
      new THREE.Vector3(2.3, 0.48, site.lane),
      new THREE.Vector3(site.stop[0] - 2.2, 0.48, site.lane),
...
function Conveyor({ position, length, rotation = 0, slope = 0 }) {
  ...
  return (
    <group position={position} rotation={[0, rotation, slope]}>
      <mesh receiveShadow>
        <boxGeometry args={[length, 0.16, 1.02]} />
      ...
      {rollers.map((x) => (
        <mesh key={x} position={[x, 0.15, 0]} rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry args={[0.06, 0.06, 0.88, 10]} />
...
function TunnelWasher() {
  return (
    <group position={[-15.15, 0.3, 0]}>
      <mesh position={[0, 0.18, -1.23]} castShadow>
        <boxGeometry args={[3.45, 0.36, 0.62]} />
      ...
      {[-1.45, 1.45].map((x) => (
        <group key={x} position={[x, 1.42, 0]}>
          <mesh castShadow>
            <boxGeometry args={[0.52, 2.5, 3]} />
            <meshStandardMaterial color={colors.cream} metalness={0.1} roughness={0.62} />
          </mesh>
          <mesh position={[x < 0 ? -0.27 : 0.27, 0, 0]} rotation={[0, Math.PI / 2, 0]}>
            <ringGeometry args={[0.57, 0.72, 28]} />
            <meshStandardMaterial color={colors.steel} metalness={0.62} roughness={0.25} side={THREE.DoubleSide} />
          </mesh>
        </group>
      ))}
      <RoundedBox args={[3.45, 0.55, 3]} radius={0.18} smoothness={4} position={[0, 2.72, 0]} castShadow>
        <meshStandardMaterial color="#d9e1dc" roughness={0.6} metalness={0.12} />
      </RoundedBox>
      <mesh position={[0, 2.99, 0]}>
        <boxGeometry args={[2.2, 0.06, 2.25]} />
        <meshStandardMaterial color={colors.blue} />
      </mesh>
      {[-0.62, 0, 0.62].map((x, index) => (
        <group key={x} position={[x, 2.75, 1.53]}>
          <mesh>
            <circleGeometry args={[0.13, 18]} />
            <meshStandardMaterial color={index === 1 ? colors.coral : colors.green} emissive={index === 1 ? colors.coral : colors.green} emissiveIntensity={0.25} />
          </mesh>
      ...
      <mesh position={[0, 1.48, 1.54]} castShadow>
        <boxGeometry args={[1.45, 0.58, 0.1]} />
        <meshStandardMaterial color={colors.ink} />
      </mesh>
      <mesh position={[0, 1.48, 1.605]}>
        <boxGeometry args={[1.18, 0.065, 0.015]} />
